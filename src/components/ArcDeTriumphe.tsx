...
export default function ArcDeTriomphe({
...
}: ObstacleProps)
 {
  ...
  const [ref1] = useCylinder<THREE.Mesh>(() => ({
    ...
    position: new CANNON.Vec3(...position).vadd(new CANNON.Vec3(...pillar1Position)).toArray(),
    type: "Static",
  }), useRef(null));
  const [ref2] = useCylinder<THREE.Mesh>(() => ({
    args: colliderBoxScale,
    position: new CANNON.Vec3(...position).vadd(new CANNON.Vec3(...pillar2Position)).toArray(),
    type: "Static",
  }), useRef(null));
  const [ref3] = useCylinder<THREE.Mesh>(() => ({
    args: colliderBoxScale,
    position: new CANNON.Vec3(...position).vadd(new CANNON.Vec3(...pillar3Position)).toArray(),
    type: "Static",
  }), useRef(null));
  const [ref4] = useCylinder<THREE.Mesh>(() => ({
    args: colliderBoxScale,
    position: new CANNON.Vec3(...position).vadd(new CANNON.Vec3(...pillar4Position)).toArray(),
    type: "Static",
  }), useRef(null));
  const [ref5] = useCylinder<THREE.Mesh>(() => ({
    args: colliderBoxScale,
    position: new CANNON.Vec3(...position).vadd(new CANNON.Vec3(...pillar5Position)).toArray(),
    type: "Static",
  }), useRef(null));
  const [ref6] = useCylinder<THREE.Mesh>(() => ({
    args: colliderBoxScale,
    position: new CANNON.Vec3(...position).vadd(new CANNON.Vec3(...pillar6Position)).toArray(),
    type: "Static",
  }), useRef(null));
  const [ref7] = useCylinder<THREE.Mesh>(() => ({
    args: colliderBoxScale,
    position: new CANNON.Vec3(...position).vadd(new CANNON.Vec3(...pillar7Position)).toArray(),
    type: "Static",
  }), useRef(null));
  const [ref8] = useCylinder<THREE.Mesh>(() => ({
    args: colliderBoxScale,
    position: new CANNON.Vec3(...position).vadd(new CANNON.Vec3(...pillar8Position)).toArray(),
    type: "Static",
  }), useRef(null));
  ...
  return (
    <group>
      <AdT scale={thingScale} position={new CANNON.Vec3(...thingPosition).vadd(new CANNON.Vec3(...position)).toArray()} 
